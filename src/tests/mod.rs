...
mod tests {
    ...
    fn find_output_file(output_dir: &Path, prefix: &str) -> Option<PathBuf> {
        ...
        let mut entries_with_time = entries
            ...
            .filter_map(|e| {
                e.metadata()
                    .ok()
                    .and_then(|m| m.modified().ok().map(|time| (e.path(), time)))
            })
            .collect::<Vec<_>>();
    ...
    fn test_name_pattern_filtering() {
        ...
        assert!(
            !content.contains("helper.h"),
            "Output should not contain helper.h"
        );
        assert!(
        ...
        for header in file_headers {
            assert!(
                header.contains("test") && header.contains(".c"),
                "Found file not matching pattern: {}",
                header
            );
        }
    }
    ...
    fn test_file_types_filtering() {
        ...
        assert!(
            !content.contains("test1.c"),
            "Output should not contain test1.c"
        );
        assert!(
            !content.contains("test2.c"),
            "Output should not contain test2.c"
        );
        assert!(
            content.contains("helper.h"),
            "Output should contain helper.h"
        );
        assert!(
            !content.contains("readme.md"),
            "Output should not contain readme.md"
        ...
        for header in file_headers {
            assert!(
                header.contains(".h"),
                "Found file without .h extension: {}",
                header
            );
        }
    }
    ...
    fn test_recursive_directory_processing() {
        ...
        // Find the generated output file
        let output_file =
            find_output_file(&output_dir, "recursive_test_").expect("No output file was generated");
        ...
        assert!(content.contains("test1.c"), "Output should contain test1.c");
        assert!(
            content.contains("nested.c"),
            "Output should contain nested.c"
        );
        ...
        assert!(
            !content.contains("test1.h"),
            "Output should not contain test1.h"
        );
        assert!(
            !content.contains("nested.h"),
            "Output should not contain nested.h"
        );
        assert!(
            !content.contains("notes.txt"),
            "Output should not contain notes.txt"
        );
        assert!(
            !content.contains("readme.md"),
            "Output should not contain readme.md"
    ...
    fn test_dotfile_handling() {
        ...
        assert!(
            output_without_d.status.success(),
            "llm_globber failed: {}",
        ...
        assert!(
            content_without_d.contains("regular.txt"),
            "Output should contain regular.txt"
        );
        assert!(
            !content_without_d.contains(".dotfile"),
            "Output should not contain .dotfile"
        );
        assert!(
            !content_without_d.contains(".config"),
            "Output should not contain .config"
        );
        ...
        assert!(
            content_with_d.contains("regular.txt"),
            "Output should contain regular.txt"
        );
        assert!(
            content_with_d.contains(".dotfile"),
            "Output should contain .dotfile"
        );
        assert!(
            content_with_d.contains(".config"),
    ...
    fn test_verbose_and_quiet_modes() {
        ...
        assert!(
            stderr_verbose.contains("INFO") || stderr_verbose.contains("DEBUG"),
        ...
        assert!(
            ...
            "Quiet mode should not produce any output, but got: {}",
            stderr_quiet
        );
        ...
        assert!(
            !stderr_default.contains("INFO:") && !stderr_default.contains("DEBUG:"),
            "Default mode should not show INFO or DEBUG messages, but got: {}",
            stderr_default
        );
    }
    ...
    fn test_basic_functionality() {
        ...
        // Find the generated output file
        let output_file =
            find_output_file(&output_dir, "basic_test_").expect("No output file was generated");
        ...
        assert!(
            content.contains("This is a C file"),
            "Output should contain C file content"
        );
        assert!(
            content.contains("This is a header file"),
            "Output should contain header file content"
        );
        ...
        assert!(
            c_pos < h_pos,
            "test1.c should appear before test1.h in the output"
        );
    }
}
